...
inline size_t delete_files_relative(
    ...
    const UnorderedSet<std::string>& relative_file_paths) {
  size_t actually_deleted = 0;
  for (const auto& f : UnorderedIterable(relative_file_paths)) {
...
inline size_t delete_files_absolute(
    const UnorderedSet<std::string>& absolute_file_paths) {
  size_t actually_deleted = 0;
  for (const auto& path : UnorderedIterable(absolute_file_paths)) {
