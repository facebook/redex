 ...
 * referenced
 */
std::unordered_set<std::string> extract_classes_from_manifest(
  ...
  do {
    ...
    if (type == android::ResXMLParser::START_TAG) {
      ...
      if (tag == activity || tag == application || tag == provider ||
          tag == receiver || tag == service || tag == instrumentation) {
        ...
        std::string classname =
            get_string_attribute_value(parser, target_activity);
        if (classname.size()) {
          result.insert(dotname_to_dexname(classname));
        }
