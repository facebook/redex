...
class Asset {
 public:
   ...
   * asset is compressed).
   */
  virtual int openFileDescriptor(off64_t* outStart,
                                 off64_t* outLength) const = 0;
 ...
 protected:
  ...
  off64_t handleSeek(off64_t offset,
                     int whence,
                     off64_t curPosn,
                     off64_t maxPosn);
...
class _FileAsset : public Asset {
 public:
   ...
   */
  status_t openChunk(const char* fileName,
                     int fd,
                     off64_t offset,
                     size_t length);
...
class _CompressedAsset : public Asset {
 ...
 private:
  ...
  class StreamingZipInflater* mZipInflater; // for streaming large compressed
                                            // assets
