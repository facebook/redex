...
struct __assertChar16Size {
  static_assert(sizeof(char16_t) == sizeof(uint16_t),
                "char16_t is not 16 bits");
  static_assert(alignof(char16_t) == alignof(uint16_t),
                "char16_t is not 16-bit aligned");
};
#endif
...
struct alignas(uintptr_t) Res_png_9patch {
  ...
  static void* serialize(const Res_png_9patch& patchHeader,
                         const int32_t* xDivs,
                         const int32_t* yDivs,
  ...
  inline int32_t* getXDivs() const {
    return reinterpret_cast<int32_t*>(reinterpret_cast<uintptr_t>(this) +
                                      xDivsOffset);
  }
...
class ResStringPool {
 public:
  ...
  inline const char16_t* stringAt(const ResStringPool_ref& ref,
                                  size_t* outLen) const {
...
class ResXMLParser {
 public:
  ...
  ssize_t indexOfAttribute(const char16_t* ns,
                           size_t nsLen,
                           const char16_t* attr,
...
struct ResTable_type {
  ...
  enum : uint32_t { NO_ENTRY = 0xFFFFFFFF };
...
/**
 * Holds the shared library ID table. Shared libraries are assigned package IDs
 * at build time, but they may be loaded in a different order, so we need to
 * maintain a mapping of build-time package ID to run-time assigned package ID.
 *
 * Dynamic references are not currently supported in overlays. Only the base
 * package may have dynamic references.
 */
class DynamicRefTable {
 public:
...
class ResTable {
 public:
  ResTable();
  ResTable(const void* data,
           size_t size,
           const int32_t cookie,
           bool copyData = false);
  ~ResTable();

  status_t add(const void* data,
               size_t size,
               const int32_t cookie = -1,
               bool copyData = false);
  status_t add(const void* data,
               size_t size,
  ...
  bool getResourceName(uint32_t resID,
                       bool allowUtf8,
   ...
   */
  ssize_t getResource(uint32_t resID,
                      Res_value* outValue,
                      bool mayBeBag = false,
                      uint16_t density = 0,
  ...
  inline ssize_t getResource(const ResTable_ref& res,
                             Res_value* outValue,
  ...
  const char16_t* valueToString(const Res_value* value,
                                size_t stringBlock,
  ...
  ssize_t getBagLocked(uint32_t resID,
  ...
  class Theme {
   public:
     ...
     * @return ssize_t Either a >= 0 table index or a negative error code.
     */
    ssize_t getAttribute(uint32_t resID,
                         Res_value* outValue,
                         uint32_t* outTypeSpecFlags = NULL) const;
     ...
     */
    ssize_t resolveAttributeReference(
  ...
  uint32_t identifierForName(const char16_t* name,
                             size_t nameLen,
  ...
  static bool expandResourceRef(const char16_t* refStr,
                                size_t refLen,
                                String16* outPackage,
                                String16* outType,
                                String16* outName,
  ...
  class Accessor {
   public:
    ...
    virtual uint32_t getCustomResourceWithCreation(
  ...
  void getConfigurations(Vector<ResTable_config>* configs,
                         bool ignoreMipmap = false) const;
  ...
  static bool getIdmapInfo(const void* idmap,
                           size_t size,
                           uint32_t* pVersion,
                           uint32_t* pTargetCrc,
  ...
  // Marks the entries (across each config) for the given resource ID as
  // deleted. Only takes effect during serialization (any deleted rows will be
  // skipped).
  ...
  // given config.
  void defineNewType(String8 type_name,
                     uint8_t type_id,
  ...
  void remapReferenceValuesForResource(uint32_t resID,
  ...
  void inlineReferenceValuesForResource(uint32_t resID,
  ...
  // ResTable::getResource, which fails for roughly 20% of resources and does
  // not handle complex (bag) values well. Note that we also return the parent
  // of bag values as a virtual TYPE_REFERENCE Res_value to reflect the
  // relationship, along with a virtual TYPE_ATTRIBUTE for the 'key' of each bag
  // entry value.
  void getAllValuesForResource(uint32_t resourceId,
 ...
 private:
  ...
  void collectValuesInConfig(Vector<Res_value>& values,
                             const ResTable_entry* ent,
                             uint32_t typeSize) const;

  void collectAllConfigs(Vector<ResTable_config>* configs,
                         const Type* type) const;

  bool tryGetConfigEntry(int entryIndex,
                         const ResTable_type* type,
                         const ResTable_entry** ent) const;

  status_t addInternal(const void* data,
                       size_t size,
                       const void* idmapData,
                       size_t idmapDataSize,
                       const int32_t cookie,
  ...
  status_t getEntry(const PackageGroup* packageGroup,
                    int typeIndex,
                    int entryIndex,
                    const ResTable_config* config,
                    Entry* outEntry) const;

  uint32_t findEntry(const PackageGroup* group,
                     ssize_t typeIndex,
                     const char16_t* name,
                     size_t nameLen,
  ...
  void serializeSingleResType(Vector<char>& output,
