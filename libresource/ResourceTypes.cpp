...
// TODO: This code uses 0xFFFFFFFF converted to bag_set* as a sentinel value.
// This is bad practice.

// Standard C isspace() is only required to look at the low byte of its input,
// so produces incorrect results for UTF-16 characters.  For safety's sake,
// assume that any high-byte UTF-16 code point is not whitespace.
inline int isspace16(char16_t c) { return (c < 0x0080 && isspace(c)); }
...
// range checked; guaranteed to NUL-terminate within the stated number of
// available slots NOTE: if this truncates the dst string due to running out of
// space, no attempt is made to avoid splitting surrogate pairs.
  ...
  if (headerSize >= minSize) {
    if (headerSize <= size) {
      if (((headerSize | size) & 0x3) == 0) {
        ...
        ALOGW("%s data size 0x%x extends beyond resource end %p.", name, size,
    ...
    ALOGW("%s size 0x%x is smaller than header size 0x%x.", name, size,
          headerSize);
    return BAD_TYPE;
  }
...
static void fill9patchOffsets(Res_png_9patch* patch) {
  ...
  patch->colorsOffset =
      patch->yDivsOffset + (patch->numYDivs * sizeof(int32_t));
}
...
size_t Res_png_9patch::serializedSize() const {
  ...
  // 4 * int32_t
  // 3 * uint32_t
  return 32 + numXDivs * sizeof(int32_t) + numYDivs * sizeof(int32_t) +
         numColors * sizeof(uint32_t);
}

void* Res_png_9patch::serialize(const Res_png_9patch& patch,
                                const int32_t* xDivs,
                                const int32_t* yDivs,
...
static bool assertIdmapHeader(const void* idmap, size_t size) {
  ...
  if (magic != IDMAP_MAGIC) {
    ALOGW("idmap: no magic found in header (is 0x%08x, expected 0x%08x)", magic,
          IDMAP_MAGIC);
    return false;
  }

  const uint32_t version =
      htodl(*(reinterpret_cast<const uint32_t*>(idmap) + 1));
 ...
 public:
  ...
  size_t byteSize() const {
    ...
    return (sizeof(uint16_t) * 4) +
           (sizeof(uint32_t) * static_cast<size_t>(entryCount));
  }
...
status_t parseIdmap(const void* idmap,
                    size_t size,
                    uint8_t* outPackageId,
  ...
  const uint16_t* data = reinterpret_cast<const uint16_t*>(
      reinterpret_cast<const uint8_t*>(idmap) +
...
void rewriteSize(Vector<char>& cVec, size_t initSize) {
  ...
  size_t sizeIndex = initSize // Header begins here
...
ResStringPool::~ResStringPool() { uninit(); }
...
void ResStringPool::appendString(String8 s) { mAppendedStrings.push_back(s); }

size_t ResStringPool::appendedStringCount() { return mAppendedStrings.size(); }

void ResStringPool::serializeWithAdditionalStrings(Vector<char>& cVec) {
  LOG_FATAL_IF(mHeader->styleCount > 0,
...
status_t ResStringPool::setTo(const void* data, size_t size, bool copyData) {
  ...
  if (mHeader->header.headerSize > mHeader->header.size ||
    ...
    ALOGW(
        "Bad string block: header size %d or total size %d is larger than data "
        "size %d\n",
  ...
  if (mHeader->styleCount > 0) {
    ...
    mStyles = (const uint32_t*)(((const uint8_t*)data) + mHeader->stylesStart);
    ...
    const ResStringPool_span endSpan = {{htodl(ResStringPool_span::END)},
...
status_t ResStringPool::getError() const { return mError; }
...
const char16_t* ResStringPool::stringAt(size_t idx, size_t* u16len) const {
  if (mError == NO_ERROR && idx < mHeader->stringCount) {
    ...
    const uint32_t off =
        mEntries[idx] / (isUTF8 ? sizeof(uint8_t) : sizeof(uint16_t));
        ...
        *u16len = decodeLength(&u8str);
        ...
        if ((uint32_t)(u8str + u8len - strings) < mStringPoolSize) {
          ...
          if (mCache == NULL) {
            ...
            ALOGW(
                "CREATING STRING CACHE OF %zu bytes",
                static_cast<size_t>(mHeader->stringCount * sizeof(char16_t**)));
#endif
            mCache =
                (char16_t**)calloc(mHeader->stringCount, sizeof(char16_t**));
            if (mCache == NULL) {
              ALOGW(
                  "No memory trying to allocate decode cache table of %d "
                  "bytes\n",
          ...
          if (actualLen < 0 || (size_t)actualLen != *u16len) {
            ALOGW(
                "Bad string block: string #%lld decoded length is not correct "
                "%lld vs %llu\n",
          ...
          if (!u16str) {
            ALOGW(
                "No memory when trying to allocate decode cache for string "
                "#%d\n",
                (int)idx);
            return NULL;
          }
          ...
          ALOGW(
              "Bad string block: string #%lld extends to %lld, past end at "
              "%lld\n",
...
const ResStringPool_span* ResStringPool::styleAt(
...
const ResStringPool& ResXMLParser::getStrings() const { return mTree.mStrings; }
...
const char16_t* ResXMLParser::getAttributeNamespace(size_t idx,
                                                    size_t* outLen) const {
...
const char* ResXMLParser::getAttributeNamespace8(size_t idx,
                                                 size_t* outLen) const {
  int32_t id = getAttributeNamespaceID(idx);
  // printf("attribute namespace=%d  idx=%d  event=%p\n", id, idx, mEventCode);
  if (kDebugXMLNoisy) {
...
const char16_t* ResXMLParser::getAttributeName(size_t idx,
                                               size_t* outLen) const {
...
const char16_t* ResXMLParser::getAttributeStringValue(size_t idx,
                                                      size_t* outLen) const {
...
ResXMLTree_attribute* ResXMLParser::getAttributePointer(size_t idx) const {
  if (mEventCode == START_TAG) {
    const ResXMLTree_attrExt* tag = (const ResXMLTree_attrExt*)mCurExt;
    if (idx < dtohs(tag->attributeCount)) {
      return (ResXMLTree_attribute*)(((const uint8_t*)tag) +
                                     dtohs(tag->attributeStart) +
                                     (dtohs(tag->attributeSize) * idx));
    }
  }
...
ssize_t ResXMLParser::indexOfAttribute(const char16_t* ns,
                                       size_t nsLen,
                                       const char16_t* attr,
  ...
  if (mEventCode == START_TAG) {
    ...
    if (mTree.mStrings.isUTF8()) {
      ...
      for (size_t i = 0; i < N; i++) {
        ...
        if (kDebugStringPoolNoisy) {
          ALOGI("  curNs=%s (%zu), curAttr=%s (%zu)", curNs, curNsLen, curAttr,
                curAttrLen);
        }
...
ResXMLParser::event_code_t ResXMLParser::nextNode() {
  ...
  do {
    const ResXMLTree_node* next =
        (const ResXMLTree_node*)(((const uint8_t*)mCurNode) +
                                 dtohl(mCurNode->header.size));
    if (kDebugXMLNoisy) {
    ...
    if ((totalSize - headerSize) < minExtSize) {
      ALOGW(
          "Bad XML block: header type 0x%x in node at 0x%x has size %d, need "
          "%d\n",
          (int)dtohs(next->header.type),
          (int)(((const uint8_t*)next) - ((const uint8_t*)mTree.mHeader)),
...
status_t ResXMLTree::setTo(const void* data, size_t size, bool copyData) {
  ...
  if (dtohs(mHeader->header.headerSize) > mSize || mSize > size) {
    ALOGW(
        "Bad XML block: header size %d or total size %d is larger than data "
        "size %d\n",
  ...
  while (((const uint8_t*)chunk) < (mDataEnd - sizeof(ResChunk_header)) &&
    ...
    status_t err =
        validate_chunk(chunk, sizeof(ResChunk_header), mDataEnd, "XML");
    if (err != NO_ERROR) {
      mError = err;
      goto done;
    }
    ...
    if (type == RES_STRING_POOL_TYPE) {
      ...
      mResIds = (uint32_t*)(((const uint8_t*)chunk) + dtohs(chunk->headerSize));
      mNumResIds =
...
status_t ResXMLTree::getError() const { return mError; }
...
status_t ResXMLTree::validateNode(const ResXMLTree_node* node) const {
  ...
  status_t err = validate_chunk(&node->header, sizeof(ResXMLTree_node),
  ...
  if (err >= NO_ERROR) {
    ...
    if ((size >= headerSize + sizeof(ResXMLTree_attrExt)) &&
      ...
      ALOGW(
          "Bad XML block: node attributes use 0x%x bytes, only have 0x%x "
          "bytes\n",
...
/* static */ void packLanguageOrRegion(const char* in,
                                       const char base,
                                       char out[2]) {
  if (in[2] == 0 || in[2] == '-') {
    out[0] = in[0];
...
/* static */ inline int compareLocales(const ResTable_config& l,
                                       const ResTable_config& r) {
  if (l.locale != r.locale) {
    // NOTE: This is the old behaviour with respect to comparison orders.
    // The diff value here doesn't make much sense (given our bit packing
    // scheme) but it's stable, and that's all we need.
...
int ResTable_config::diff(const ResTable_config& o) const {
  ...
  if (smallestScreenWidthDp != o.smallestScreenWidthDp)
    diffs |= CONFIG_SMALLEST_SCREEN_SIZE;
...
int ResTable_config::isLocaleMoreSpecificThan(const ResTable_config& o) const {
  ...
  const int score =
      ((localeScript[0] != 0) ? 1 : 0) + ((localeVariant[0] != 0) ? 2 : 0);

  const int oScore =
      ((o.localeScript[0] != 0) ? 1 : 0) + ((o.localeVariant[0] != 0) ? 2 : 0);
  ...
  if (requested) {
    ...
    if (screenSizeDp || o.screenSizeDp) {
      ...
      if (kDebugTableSuperNoisy) {
        ALOGI(
            "Comparing this %dx%d to other %dx%d in %dx%d: myDelta=%d "
            "otherDelta=%d",
            screenWidthDp, screenHeightDp, o.screenWidthDp, o.screenHeightDp,
            requested->screenWidthDp, requested->screenHeightDp, myDelta,
            otherDelta);
      }
    ...
    if (screenType || o.screenType) {
      if (density != o.density) {
        // Use the system default density (DENSITY_MEDIUM, 160dpi) if none
        // specified.
        const int thisDensity =
            density ? density : int(ResTable_config::DENSITY_MEDIUM);
        const int otherDensity =
            o.density ? o.density : int(ResTable_config::DENSITY_MEDIUM);
...
bool ResTable_config::match(const ResTable_config& settings) const {
  ...
  if (locale != 0) {
    ...
    if (language[0] != 0 && (language[0] != settings.language[0] ||
                             language[1] != settings.language[1])) {
      return false;
    }

    if (country[0] != 0 && (country[0] != settings.country[0] ||
...
/* static */ inline bool assignLocaleComponent(ResTable_config* config,
  ...
  case 2:
  case 3:
    config->language[0] ? config->packRegion(start)
...
String8 ResTable_config::toString() const {
  ...
  if ((uiMode & MASK_UI_MODE_NIGHT) != 0) {
    ...
    case ResTable_config::UI_MODE_NIGHT_YES:
      ...
      break;
    default:
      res.appendFormat("uiModeNight=%d", dtohs(uiMode & MASK_UI_MODE_NIGHT));
      break;
    }
  }
...
struct ResTable::Type {
  ...
  void serialize(Vector<char>& cVec) {
    ...
    for (size_t k = 0; k < configs.size(); k++) {
      ...
      size_t skippedHeaderFields = 2 // type
                                   + 2 // header size
                                   + 4 // chunk size
                                   + 1 // id
      ...
      memcpy((char*)&(cVec[initSize + skippedHeaderFields]),
             (char*)(&newRowCount),
             4);
      uint32_t newOffsetToEntries = (4 * newRowCount) + type->header.headerSize;
      memcpy((char*)&(cVec[initSize + skippedHeaderFields + 4]),
      ...
      if (lastRealOffset != ResTable_type::NO_ENTRY) {
        size_t entrySize =
            type->header.size - type->entriesStart - lastRealOffset;
        entrySizes.push_back(entrySize);
      }
      ...
      for (size_t i = 0; i < entryCount; ++i) {
        ...
        if (offset != ResTable_type::NO_ENTRY) {
          ...
          if (wasDeleted) {
            ...
            const ResTable_entry* const entry =
                reinterpret_cast<const ResTable_entry*>(
                    reinterpret_cast<const uint8_t*>(type) + offset +
                    type->entriesStart);
...
struct ResTable::Package {
  Package(ResTable* _owner,
          const Header* _header,
          const ResTable_package* _package)
...
status_t ResTable::Theme::applyStyle(uint32_t resID, bool force) {
  ...
  while (bag < end) {
    ...
    if (kDebugTableNoisy) {
      ALOGV("Attr 0x%08x: type=0x%x, data=0x%08x; curType=0x%x", attrRes,
            bag->map.value.dataType, bag->map.value.data,
...
ssize_t ResTable::Theme::getAttribute(uint32_t resID,
                                      Res_value* outValue,
      ...
      if (pi != NULL) {
        ...
        if (t <= Res_MAXTYPE) {
          ...
          if (e < ti.numEntries) {
            ...
            if (kDebugTableTheme) {
              ALOGI("Theme value: type=0x%x, data=0x%08x", te.value.dataType,
                    te.value.data);
            }
...
ssize_t ResTable::Theme::resolveAttributeReference(
    Res_value* inOutValue,
    ssize_t blockIndex,
    uint32_t* outLastRef,
    ...
    for (size_t j = 0; j <= Res_MAXTYPE; j++) {
      ...
      for (size_t k = 0; k < ti.numEntries; k++) {
        ...
        ALOGI("      0x%08x: t=0x%x, d=0x%08x (block=%d)\n",
              (int)Res_MAKEID(i, j, k), te.value.dataType, (int)te.value.data,
              (int)te.stringBlock);
      }
    }
  }
...
ResTable::ResTable() : mError(NO_INIT), mNextPackageId(2) {
...
ResTable::ResTable(const void* data,
                   size_t size,
...
status_t ResTable::add(const void* data,
                       size_t size,
                       const int32_t cookie,
                       bool copyData) {
...
status_t ResTable::add(Asset* asset, const int32_t cookie, bool copyData) {
  ...
  return addInternal(data, static_cast<size_t>(asset->getLength()), NULL, 0,
                     cookie, copyData);
}

status_t ResTable::add(Asset* asset,
                       Asset* idmapAsset,
                       const int32_t cookie,
                       bool copyData) {
  const void* data = asset->getBuffer(true);
  ...
  return addInternal(data, static_cast<size_t>(asset->getLength()), idmapData,
                     idmapSize, cookie, copyData);
}
...
status_t ResTable::addInternal(const void* data,
                               size_t dataSize,
                               const void* idmapData,
                               size_t idmapDataSize,
                               const int32_t cookie,
  ...
  if (kDebugLoadTableNoisy) {
    ALOGV(
        "Adding resources to ResTable: data=%p, size=%zu, cookie=%d, copy=%d "
        "idmap=%p\n",
  ...
  if (dtohs(header->header->header.headerSize) > header->size ||
    ...
    ALOGW(
        "Bad resource table: header size 0x%x or total size 0x%x is larger "
        "than data size 0x%x\n",
  ...
  if (((dtohs(header->header->header.headerSize) | header->size) & 0x3) != 0) {
    ALOGW(
        "Bad resource table: header size 0x%x or total size 0x%x is not on an "
        "integer boundary\n",
  ...
  while (((const uint8_t*)chunk) <=
             (header->dataEnd - sizeof(ResChunk_header)) &&
         ((const uint8_t*)chunk) <= (header->dataEnd - dtohl(chunk->size))) {
    status_t err = validate_chunk(chunk, sizeof(ResChunk_header),
                                  header->dataEnd, "ResTable");
    if (err != NO_ERROR) {
      return (mError = err);
    }
    if (kDebugTableNoisy) {
    ...
    if (ctype == RES_STRING_POOL_TYPE) {
      ...
      if (curPackage >= dtohl(header->header->packageCount)) {
        ALOGW(
            "More package chunks were found than the %d declared in the "
            "header.",
  ...
  if (curPackage < dtohl(header->header->packageCount)) {
    ALOGW(
        "Fewer package chunks (%d) were found than the %d declared in the "
        "header.",
...
status_t ResTable::getError() const { return mError; }
...
bool ResTable::getResourceName(uint32_t resID,
                               bool allowUtf8,
                               resource_name* outName) const {
  if (mError != NO_ERROR) {
    return false;
  }
  ...
  if (p < 0) {
    if (Res_GETPACKAGE(resID) + 1 == 0) {
      ...
      ALOGW("No known package when getting name for resource number 0x%08x",
            resID);
    }
    return false;
  }
  if (t < 0) {
    ALOGW("No type identifier when getting name for resource number 0x%08x",
          resID);
    return false;
  }
...
ssize_t ResTable::getResource(uint32_t resID,
                              Res_value* outValue,
                              bool mayBeBag,
                              uint16_t density,
  ...
  if (p < 0) {
    if (Res_GETPACKAGE(resID) + 1 == 0) {
      ...
      ALOGW("No known package when getting value for resource number 0x%08x",
            resID);
    }
    return BAD_INDEX;
  }
  if (t < 0) {
    ALOGW("No type identifier when getting value for resource number 0x%08x",
          resID);
    return BAD_INDEX;
  }
  ...
  const PackageGroup* const grp = mPackageGroups[p];
  if (grp == NULL) {
    ALOGW("Bad identifier when getting value for resource number 0x%08x",
          resID);
    return BAD_INDEX;
  }
  ...
  if (kDebugTableNoisy) {
    ...
    printf("Found value: pkg=%zu, type=%d, str=%s, int=%d\n",
           ...
           outValue->dataType == Res_value::TYPE_STRING
               ? String8(entry.package->header->values.stringAt(outValue->data,
                                                                &len))
                     .string()
               : "",
           outValue->data);
...
ssize_t ResTable::resolveReference(Res_value* value,
                                   ssize_t blockIndex,
                                   uint32_t* outLastRef,
                                   uint32_t* inoutTypeSpecFlags,
                                   ResTable_config* outConfig) const {
  ...
  while (blockIndex >= 0 && value->dataType == Res_value::TYPE_REFERENCE &&
    ...
    uint32_t newFlags = 0;
    const ssize_t newIndex =
        getResource(value->data, value, true, 0, &newFlags, outConfig);
...
void ResTable::lock() const { mLock.lock(); }

void ResTable::unlock() const { mLock.unlock(); }

ssize_t ResTable::getBagLocked(uint32_t resID,
                               const bag_entry** outBag,
                               uint32_t* outTypeSpecFlags) const {
  if (mError != NO_ERROR) {
    return mError;
  }
  ...
  if (t < 0) {
    ALOGW("No type identifier when getting bag for resource number 0x%08x",
          resID);
    return BAD_INDEX;
  }
  ...
  if (e >= (int)NENTRY) {
    ALOGW("Entry identifier 0x%x is larger than entry count 0x%x", e,
          (int)typeConfigs[0]->entryCount);
    return BAD_INDEX;
  }
  ...
  const uint32_t parent =
      entrySize >= sizeof(ResTable_map_entry)
          ? dtohl(((const ResTable_map_entry*)entry.entry)->parent.ident)
          : 0;
  const uint32_t count =
      entrySize >= sizeof(ResTable_map_entry)
          ? dtohl(((const ResTable_map_entry*)entry.entry)->count)
          : 0;
  ...
  if (parent) {
    ...
    const ssize_t NP =
        getBagLocked(resolvedParent, &parentBag, &parentTypeSpecFlags);
  ...
  size_t curOff = (reinterpret_cast<uintptr_t>(entry.entry) -
  ...
  while (pos < count) {
    ...
    if (curOff > (dtohl(entry.type->header.size) - sizeof(ResTable_map))) {
      ALOGW("ResTable_map at %d is beyond type chunk data %d", (int)curOff,
            dtohl(entry.type->header.size));
      return BAD_TYPE;
    }
    ...
    while ((isInside = (curEntry < set->numAttrs)) &&
      ...
      if (kDebugTableNoisy) {
        ALOGI("#%zu: Keeping existing attribute: 0x%08x\n", curEntry,
              entries[curEntry].map.name.ident);
      }
      curEntry++;
    }
    ...
    if ((!isInside) || oldName != newName) {
      ...
      if (set->numAttrs >= set->availAttrs) {
        ...
        set = (bag_set*)realloc(set,
                                sizeof(bag_set) + sizeof(bag_entry) * newAvail);
        if (set == NULL) {
          return NO_MEMORY;
        }
    ...
    if (kDebugTableNoisy) {
      ALOGI(
          "Setting entry #%zu %p: block=%zd, name=0x%08d, type=%d, "
          "data=0x%08x\n",
...
uint32_t ResTable::identifierForName(const char16_t* name,
                                     size_t nameLen,
                                     const char16_t* type,
                                     size_t typeLen,
  ...
  if (name[0] == '^') {
    ...
    if (nameLen > 7) {
      if (name[1] == 'i' && name[2] == 'n' && name[3] == 'd' &&
          name[4] == 'e' && name[5] == 'x' && name[6] == '_') {
  ...
  for (size_t ig = 0; ig < NG; ig++) {
    ...
    if (strzcmp16(package, packageLen, group->name.string(),
                  group->name.size())) {
      if (kDebugTableNoisy) {
    ...
    for (size_t pi = 0; pi < packageCount; pi++) {
      ...
      do {
        ...
        const uint32_t identifier =
            findEntry(group, ti, name, nameLen, outTypeSpecFlags);
        if (identifier != 0) {
      ...
      } while (strzcmp16(attr.string(), attr.size(), targetType,
                         targetTypeLen) == 0 &&
...
uint32_t ResTable::findEntry(const PackageGroup* group,
                             ssize_t typeIndex,
                             const char16_t* name,
                             size_t nameLen,
                             uint32_t* outTypeSpecFlags) const {
...
bool ResTable::expandResourceRef(const char16_t* refStr,
                                 size_t refLen,
                                 String16* outPackage,
                                 String16* outType,
                                 String16* outName,
...
static const unit_entry unitNames[] = {
    {"px", strlen("px"), Res_value::TYPE_DIMENSION, Res_value::COMPLEX_UNIT_PX,
     1.0f},
    {"dip", strlen("dip"), Res_value::TYPE_DIMENSION,
     Res_value::COMPLEX_UNIT_DIP, 1.0f},
    {"dp", strlen("dp"), Res_value::TYPE_DIMENSION, Res_value::COMPLEX_UNIT_DIP,
     1.0f},
    {"sp", strlen("sp"), Res_value::TYPE_DIMENSION, Res_value::COMPLEX_UNIT_SP,
     1.0f},
    {"pt", strlen("pt"), Res_value::TYPE_DIMENSION, Res_value::COMPLEX_UNIT_PT,
     1.0f},
    {"in", strlen("in"), Res_value::TYPE_DIMENSION, Res_value::COMPLEX_UNIT_IN,
     1.0f},
    {"mm", strlen("mm"), Res_value::TYPE_DIMENSION, Res_value::COMPLEX_UNIT_MM,
     1.0f},
    {"%", strlen("%"), Res_value::TYPE_FRACTION,
     Res_value::COMPLEX_UNIT_FRACTION, 1.0f / 100},
    {"%p", strlen("%p"), Res_value::TYPE_FRACTION,
     Res_value::COMPLEX_UNIT_FRACTION_PARENT, 1.0f / 100},
    {NULL, 0, 0, 0, 0}};

static bool parse_unit(const char* str,
                       Res_value* outValue,
                       float* outScale,
                       const char** outEnd) {
  const char* end = str;
...
bool ResTable::stringToFloat(const char16_t* s,
                             size_t len,
                             Res_value* outValue) {
  while (len > 0 && isspace16(*s)) {
    s++;
  ...
  if (*end != 0 && !isspace((unsigned char)*end)) {
    ...
    if (parse_unit(end, outValue, &scale, &end)) {
      ...
      int32_t mantissa =
          (int32_t)((bits >> shift) & Res_value::COMPLEX_MANTISSA_MASK);
      if (neg) {
        mantissa = (-mantissa) & Res_value::COMPLEX_MANTISSA_MASK;
      }
      outValue->data |= (radix << Res_value::COMPLEX_RADIX_SHIFT) |
                        (mantissa << Res_value::COMPLEX_MANTISSA_SHIFT);
      // printf("Input value: %f 0x%016Lx, mult: %f, radix: %d, shift: %d,
      // final: 0x%08x\n",
...
bool ResTable::stringToValue(Res_value* outValue,
  ...
  bool localizationSetting =
      accessor != NULL && accessor->getLocalizationSetting();
  ...
  if (*s == '@') {
    ...
    if (len == 5 && s[1] == 'n' && s[2] == 'u' && s[3] == 'l' && s[4] == 'l') {
      ...
      outValue->data = 0;
      return true;
    } else if (len == 6 && s[1] == 'e' && s[2] == 'm' && s[3] == 'p' &&
               s[4] == 't' && s[5] == 'y') {
      ...
      if (!expandResourceRef(resourceRefName, resourceNameLen, &package, &type,
                             &name, defType, defPackage, &errorMsg)) {
        if (accessor != NULL) {
      ...
      uint32_t rid = identifierForName(
          name.string(), name.size(), type.string(), type.size(),
        ...
        if (accessor) {
          ...
          if (kDebugTableNoisy) {
            ALOGI("Incl %s:%s/%s: 0x%08x\n", String8(package).string(),
                  String8(type).string(), String8(name).string(), rid);
          }
        }
      ...
      if (accessor) {
        uint32_t rid = accessor->getCustomResourceWithCreation(
            package, type, name, createIfNotFound);
        if (rid != 0) {
          if (kDebugTableNoisy) {
          ...
          if (packageId == 0x00) {
            ...
            outValue->dataType = Res_value::TYPE_DYNAMIC_REFERENCE;
            return true;
          } else if (packageId == APP_PACKAGE_ID ||
  ...
  if (*s == '#') {
    ...
    if (!error) {
      if ((attrType & ResTable_map::TYPE_COLOR) == 0) {
        if (!canStringCoerce) {
          if (accessor != NULL) {
            accessor->reportError(accessorCookie, "Color types not allowed");
          }
          return false;
        }
      } else {
  ...
  if (*s == '?') {
    ...
    String16 package, type, name;
    if (!expandResourceRef(s + 1, len - 1, &package, &type, &name, &attr16,
                           defPackage, &errorMsg)) {
      if (accessor != NULL) {
        accessor->reportError(accessorCookie, errorMsg);
      }
    ...
    uint32_t rid = identifierForName(name.string(), name.size(), type.string(),
                                     type.size(), package.string(),
                                     package.size(), &specFlags);
    if (rid != 0) {
      if (enforcePrivate) {
        if ((specFlags & ResTable_typeSpec::SPEC_PUBLIC) == 0) {
          if (accessor != NULL) {
          ...
          if (!Res_INTERNALID(bagi->map.name.ident)) {
            ...
            if (getResourceName(bagi->map.name.ident, false, &rname)) {
                            ...
                            printf("Matching %s against %s (0x%08x)\n",
                                   ...
                                   bagi->map.name.ident);
#endif
              if (strzcmp16(start, pos - start, rname.name, rname.nameLen) ==
                  0) {
...
bool ResTable::collectString(String16* outString,
  ...
  while (p < (s + len)) {
    while (p < (s + len)) {
      ...
      if (!preserveSpaces) {
        if (quoted == 0 && isspace16(c) && (c != ' ' || isspace16(*(p + 1)))) {
          break;
        }
...
const String16 ResTable::getBasePackageName(size_t idx) const {
  ...
  LOG_FATAL_IF(idx >= mPackageGroups.size(),
               "Requested package index %d past package count %d", (int)idx,
...
size_t ResTable::getTableCount() const { return mHeaders.size(); }
...
const DynamicRefTable* ResTable::getDynamicRefTableForCookie(
...
void ResTable::collectAllConfigs(Vector<ResTable_config>* configs,
...
void ResTable::getConfigurations(Vector<ResTable_config>* configs,
                                 bool ignoreMipmap) const {
  ...
  for (size_t i = 0; i < packageCount; i++) {
    ...
    for (size_t j = 0; j < typeCount; j++) {
      ...
      for (size_t k = 0; k < numTypes; k++) {
        ...
        if (ignoreMipmap &&
            typeStrings.string8ObjectAt(type->typeSpec->id - 1) == "mipmap") {
          continue;
        }
...
void ResTable::getConfigurationsByType(size_t base_package_idx,
                                       String8 type_name,
...
StringPoolRef::StringPoolRef() : mPool(NULL), mIndex(0) {}
...
bool ResTable::getResourceFlags(uint32_t resID, uint32_t* outFlags) const {
  ...
  if (p < 0) {
    if (Res_GETPACKAGE(resID) + 1 == 0) {
      ALOGW(
          "No package identifier when getting flags for resource number 0x%08x",
          resID);
    } else {
      ALOGW("No known package when getting flags for resource number 0x%08x",
            resID);
    }
    return false;
  }
  if (t < 0) {
    ALOGW("No type identifier when getting flags for resource number 0x%08x",
          resID);
    return false;
  }
  ...
  const PackageGroup* const grp = mPackageGroups[p];
  if (grp == NULL) {
    ALOGW("Bad identifier when getting flags for resource number 0x%08x",
          resID);
    return false;
  }
...
status_t ResTable::getEntry(const PackageGroup* packageGroup,
  ...
  for (size_t i = 0; i < typeCount; i++) {
    ...
    if (typeSpec->idmapEntries.hasEntries()) {
      ...
      if (typeSpec->idmapEntries.lookup(entryIndex, &overlayEntryIndex) !=
          NO_ERROR) {
    ...
    if (static_cast<size_t>(realEntryIndex) >= typeSpec->entryCount) {
      ALOGW(
          ...
          Res_MAKEID(packageGroup->id - 1, typeIndex, entryIndex), entryIndex,
          static_cast<int>(typeSpec->entryCount));
  ...
  if (bestOffset > (dtohl(bestType->header.size) - sizeof(ResTable_entry))) {
    ALOGW("ResTable_entry at 0x%x is beyond type chunk data 0x%x", bestOffset,
          dtohl(bestType->header.size));
    return BAD_TYPE;
  }
  if ((bestOffset & 0x3) != 0) {
  ...
  if (outEntry != NULL) {
    ...
    outEntry->typeStr = StringPoolRef(
        &bestPackage->typeStrings, actualTypeIndex - bestPackage->typeIdOffset);
    outEntry->keyStr =
        StringPoolRef(&bestPackage->keyStrings, dtohl(entry->key.index));
  }
  return NO_ERROR;
}
...
status_t ResTable::parsePackage(const ResTable_package* const pkg,
  ...
  status_t err =
      validate_chunk(&pkg->header, sizeof(*pkg) - sizeof(pkg->typeIdOffset),
  ...
  while (((const uint8_t*)chunk) <= (endPos - sizeof(ResChunk_header)) &&
    ...
    if (kDebugTableNoisy) {
      ALOGV(
          "PackageChunk: type=0x%x, headerSize=0x%x, size=0x%x, pos=%p\n",
          dtohs(chunk->type), dtohs(chunk->headerSize), dtohl(chunk->size),
          (void*)(((const uint8_t*)chunk) - ((const uint8_t*)header->header)));
    }
    const size_t csize = dtohl(chunk->size);
    ...
    if (ctype == RES_TABLE_TYPE_SPEC_TYPE) {
      ...
      err = validate_chunk(&typeSpec->header, sizeof(*typeSpec), endPos,
                           "ResTable_typeSpec");
      if (err != NO_ERROR) {
        return (mError = err);
      }
      ...
      if (newEntryCount > 0) {
        ...
        if (!typeList.isEmpty()) {
          ...
          if (existingType->entryCount != newEntryCount && idmapIndex < 0) {
            ALOGW(
                "ResTable_typeSpec entry count inconsistent: given %d, "
                "previously %d",
        ...
        t->typeSpec = typeSpec;
        t->typeSpecFlags =
            (const uint32_t*)(((const uint8_t*)typeSpec) +
                              dtohs(typeSpec->header.headerSize));
        if (idmapIndex >= 0) {
      ...
      err = validate_chunk(&type->header,
                           sizeof(*type) - sizeof(ResTable_config) + 4, endPos,
                           "ResTable_type");
      if (err != NO_ERROR) {
        return (mError = err);
      }
      ...
      if (group->dynamicRefTable.entries().size() == 0) {
        status_t err =
            group->dynamicRefTable.load((const ResTable_lib_header*)chunk);
        if (err != NO_ERROR) {
          return (mError = err);
        }
      ...
      status_t err = validate_chunk(chunk, sizeof(ResChunk_header), endPos,
...
status_t DynamicRefTable::load(const ResTable_lib_header* const header) {
  ...
  const uint32_t expectedSize =
      dtohl(header->header.size) - dtohl(header->header.headerSize);
  ...
  const ResTable_lib_entry* entry =
      (const ResTable_lib_entry*)(((uint8_t*)header) +
...
status_t DynamicRefTable::addMapping(const String16& packageName,
                                     uint8_t packageId) {
...
status_t ResTable::createIdmap(const ResTable& overlay,
  ...
  if (mPackageGroups[0]->packages.size() == 0) {
    ALOGW(
        "idmap: target package has no packages in its first package group, "
        "cannot create idmap\n");
    return UNKNOWN_ERROR;
  }
  ...
  *outSize += 2 * sizeof(uint16_t);
  ...
  const ResTable_package* overlayPackageStruct =
      overlay.mPackageGroups[0]->packages[0]->package;
  char16_t tmpName[sizeof(overlayPackageStruct->name) /
                   sizeof(overlayPackageStruct->name[0])];
  strcpy16_dtoh(tmpName, overlayPackageStruct->name,
                sizeof(overlayPackageStruct->name) /
                    sizeof(overlayPackageStruct->name[0]));
  ...
  for (size_t typeIndex = 0; typeIndex < pg->types.size(); ++typeIndex) {
    ...
    for (size_t entryIndex = 0; entryIndex < typeConfigs->entryCount;
         ++entryIndex) {
      ...
      if (Res_GETTYPE(overlayResID) + 1 !=
          static_cast<size_t>(typeMap.overlayTypeId)) {
        ALOGE(
            "idmap: can't mix type ids in entry map. Resource 0x%08x maps to "
            "0x%08x"
      ...
      if (typeMap.entryOffset + typeMap.entryMap.size() < entryIndex) {
        // pad with 0xffffffff's (indicating non-existing entries) before adding
        // this entry
    ...
    if (!typeMap.entryMap.isEmpty()) {
      ...
      *outSize +=
          (4 * sizeof(uint16_t)) + (typeMap.entryMap.size() * sizeof(uint32_t));
    }
  }
...
bool ResTable::getIdmapInfo(const void* idmap,
                            size_t sizeBytes,
                            uint32_t* pVersion,
                            uint32_t* pTargetCrc,
                            uint32_t* pOverlayCrc,
                            String8* pTargetPath,
                            String8* pOverlayPath) {
  ...
  if (pOverlayPath) {
    pOverlayPath->setTo(
        reinterpret_cast<const char*>(map + 4 + 256 / sizeof(uint32_t)));
  }
  return true;
}
...
#define CHAR16_ARRAY_EQ(constant, var, len)             \
  ((len == (sizeof(constant) / sizeof(constant[0]))) && \
   (0 == memcmp((var), (constant), (len))))

float complex_value(uint32_t complex) {
  const float MANTISSA_MULT = 1.0f / (1 << Res_value::COMPLEX_MANTISSA_SHIFT);
...
uint32_t complex_unit(uint32_t complex, bool isFraction) {
  return (complex >> Res_value::COMPLEX_UNIT_SHIFT) &
...
String8 ResTable::normalizeForOutput(const char* input) {
  ...
  while (*input != '\0') {
    switch (*input) {
      // All interesting characters are in the ASCII zone, so we are making our
      // own lives easier by scanning the string one byte at a time.
    case '\\':
      ret += "\\\\";
      break;
    case '\n':
...
String8 ResTable::getString8FromIndex(ssize_t packageIndex,
...
void ResTable::getTypeNamesForPackage(ssize_t packageIndex,
...
void ResTable::print_value(const Package* pkg, const Res_value& value) const {
  if (value.dataType == Res_value::TYPE_NULL) {
    ...
    if (str8 != NULL) {
      ...
      const char16_t* str16 = pkg->header->values.stringAt(value.data, &len);
      if (str16 != NULL) {
        printf("(string16) \"%s\"\n",
    ...
    printf("(unknown type) t=0x%02x d=0x%08x (s=0x%04x r=0x%02x)\n",
           (int)value.dataType, (int)value.data, (int)value.size,
           (int)value.res0);
  }
}

// Allows SortedVector to be used as a poor man's substitute for
// std::unordered_set; internally searches for value using a binary search
// (order of log n).
template <typename T>
...
static uint32_t getRemappedEntry(uint32_t reference,
...
void ResTable::remapReferenceValuesForResource(
  ...
  for (size_t configIndex = 0; configIndex < NTC; configIndex++) {
    ...
    if ((dtohs(ent->flags) & ResTable_entry::FLAG_COMPLEX) != 0) {
      ...
      valuePtr = (Res_value*)(((const uint8_t*)ent) + esize);
    }
    ...
    if (valuePtr != nullptr &&
      ...
      for (int i = 0; i < N && mapOffset < (typeSize - sizeof(ResTable_map));
           i++) {
...
bool ResTable::tryGetConfigEntry(int entryIndex,
                                 const ResTable_type* type,
  ...
  const uint32_t* const eindex =
      (const uint32_t*)(((const uint8_t*)type) +
                        dtohs(type->header.headerSize));
  ...
  const ResTable_entry* ent =
      (const ResTable_entry*)(((const uint8_t*)type) + entriesStart +
                              thisOffset);
...
void ResTable::serializeSingleResType(Vector<char>& output,
  ...
  for (size_t i = 0; i < num_ids; i++) {
    ...
    if (entry_is_complex) {
      ...
      auto total_size = dtohs(map_entry_ptr->size) +
...
void ResTable::defineNewType(String8 type_name,
                             uint8_t type_id,
  ...
  for (size_t i = 0; i < num_ids; i++) {
    ...
    LOG_FATAL_IF(Res_GETTYPE(id) != old_type_id,
                 "Given ids must be in the same type");
    LOG_FATAL_IF(getResourcePackageIndex(id) != pkg_index,
                 "Given ids must be in the same package");

    Entry entry;
    status_t err = getEntry(pg, old_type_id, Res_GETENTRY(id), nullptr, &entry);
    LOG_FATAL_IF(err != NO_ERROR, "Entry 0x%x not found [err %d]", id, err);
  ...
  LOG_FATAL_IF(serialized_chars == nullptr,
...
void ResTable::getAllValuesForResource(uint32_t resID,
                                       Vector<Res_value>& values) const {
  this->getAllValuesForResource(resID, values, /* onlyDefault */ false);
}

void ResTable::collectValuesInConfig(Vector<Res_value>& values,
                                     const ResTable_entry* ent,
  ...
  if ((dtohs(ent->flags) & ResTable_entry::FLAG_COMPLEX) != 0) {
    ...
    valuePtr = (const Res_value*)(((const uint8_t*)ent) + esize);
...
void ResTable::getAllValuesForResource(uint32_t resID,
  ...
  if (onlyDefault) {
    ResTable_config def = {sizeof(ResTable_config)};
...
bool ResTable::areResourceValuesIdentical(uint32_t resourceId1,
...
void ResTable::print(bool inclValues) const {
  ...
  for (size_t pgIndex = 0; pgIndex < pgCount; pgIndex++) {
    ...
    printf("Package Group %d id=0x%02x packageCount=%d name=%s\n", (int)pgIndex,
           pg->id, (int)pg->packages.size(), String8(pg->name).string());

    const KeyedVector<String16, uint8_t>& refEntries =
        pg->dynamicRefTable.entries();
    ...
    for (size_t pkgIndex = 0; pkgIndex < pkgCount; pkgIndex++) {
      ...
      char16_t
          tmpName[sizeof(pkg->package->name) / sizeof(pkg->package->name[0])];
    ...
    for (size_t typeIndex = 0; typeIndex < pg->types.size(); typeIndex++) {
      ...
      printf("    type %d configCount=%d entryCount=%d\n", (int)typeIndex,
             (int)NTC, (int)typeConfigs->entryCount);
      ...
      for (size_t configIndex = 0; configIndex < NTC; configIndex++) {
        ...
        for (size_t entryIndex = 0; entryIndex < entryCount; entryIndex++) {
          ...
          if ((thisOffset + esize) > typeSize) {
            printf(
                "ResTable_entry OUT OF BOUNDS: 0x%x+0x%x+%p (size is 0x%x)\n",
          ...
          if ((dtohs(ent->flags) & ResTable_entry::FLAG_COMPLEX) != 0) {
            ...
            valuePtr = (const Res_value*)(((const uint8_t*)ent) + esize);
            value.copyFrom_dtoh(*valuePtr);
            printf("t=0x%02x d=0x%08x (s=0x%04x r=0x%02x)", (int)value.dataType,
                   (int)value.data, (int)value.size, (int)value.res0);
          }
          ...
          if (inclValues) {
            if (valuePtr != NULL) {
              ...
              const uint8_t* baseMapPtr = (const uint8_t*)ent;
              size_t mapOffset = esize;
              const ResTable_map* mapPtr =
                  (ResTable_map*)(baseMapPtr + mapOffset);
              const uint32_t parent = dtohl(bagPtr->parent.ident);
              uint32_t resolvedParent = parent;
              ...
              for (int i = 0;
                   i < N && mapOffset < (typeSize - sizeof(ResTable_map));
                   i++) {
                printf("          #%i (Key=0x%08x): ", i,
                       dtohl(mapPtr->name.ident));
                value.copyFrom_dtoh(mapPtr->value);
                print_value(typeConfigs->package, value);
                const size_t size = dtohs(mapPtr->value.size);
                mapOffset += size + sizeof(*mapPtr) - sizeof(mapPtr->value);
