...
void OriginalNamePass::run_pass(DexStoresVector& stores,
  ...
  for (const auto& it : to_annotate) {
    ...
    always_assert_log(
        ...
        "Encountered a generated class %s in %s. The class may be generated by "
        "merging other targeting classes by ClassMerging service, disable the "
        "merging to fix the error.",
        SHOW(cls->get_deobfuscated_name_or_empty()), SHOW(this->name()));
    auto external_name =
        java_names::internal_to_external(cls->get_deobfuscated_name_or_empty());
