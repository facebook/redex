 ...
 */
int rename_scope_ref(DexMethod* meth,
 ...
 */
int rename_scope(const VirtualScope* scope,
                 const RefsMap& def_refs,
                 DexString* name) {
  int renamed = 0;
...
struct VirtualRenamer {
  VirtualRenamer(const ClassScopes& class_scopes, const RefsMap& def_refs)
      : class_scopes(class_scopes), def_refs(def_refs) {}
 ...
 */
int VirtualRenamer::rename_virtual_scopes(const DexType* type,
                                          int& seed) const {
  int renamed = 0;
  const auto cls = type_class(type);
  ...
  if (cls != nullptr && !cls->is_external()) {
    ...
    for (auto& scope : scopes) {
      ...
      if (is_impl_scope(scope)) {
        TRACE(OBFUSCATE, 5, "Impl scope %s\n", SHOW(scope->methods[0].first));
        continue;
      }
