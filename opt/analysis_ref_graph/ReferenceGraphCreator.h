...
class CreateReferenceGraphPass : public Pass {
 ...
 private:
  ...
  using refs_t =
      std::unordered_map<const DexClass*,
  ...
  void build_super_and_interface_refs(const Scope& scope, refs_t& class_refs);

  template <class T>
  void get_annots(const T* thing_with_annots,
                  const DexClass* enclosing_class,
                  refs_t& class_refs);

  MethodWalkerFn method_ref_builder(const Scope& scope, refs_t& class_refs);

  FieldWalkerFn field_ref_builder(const Scope& scope, refs_t& class_refs);

  void build_class_annot_refs(const Scope& scope, refs_t& class_refs);

  MethodWalkerFn method_annot_ref_builder(const Scope& scope,
                                          refs_t& class_refs);

  FieldWalkerFn field_annot_ref_builder(const Scope& scope, refs_t& class_refs);

  MethodWalkerFn exception_ref_builder(const Scope& scope, refs_t& class_refs);

  InstructionWalkerFn instruction_ref_builder(const Scope& scope,
                                              refs_t& class_refs);
  ...
  void createAndOutputRefGraph(DexStore& store,
