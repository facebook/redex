...
VdexFile::VdexFile(VdexFileHeader& header, ConstBuffer buf)
    : header_(header) {
  auto remaining_dexes_buf = buf.slice(sizeof(VdexFileHeader) + size_of_checksums_section(header));
  for (size_t dex_index = 0; dex_index < header.number_of_dex_files_; ++dex_index) {
...
void VdexFile::print() const {
  ...
  for (const auto& e : dexes_) {
    print_dex_opcodes(reinterpret_cast<const uint8_t*>(e.ptr), dex_headers_[index].file_size);
    index++;
  }
}
