...
class NativeOutlinerCodegen : public Tool {
 public:
  NativeOutlinerCodegen()
      : Tool("native-outliner-codegen", "codegen helper for native outliner") {}
 ...
 private:
  std::string generateOutlineThrowsCases(
      const OutlinedThrows* outlined_throws) {
    std::string cases;
    int idx = 0;
    ...
    case %0d:
      ...
      break;
                  ...
                  outlined_throw->type()->c_str() %
                  outlined_throw->msg()->c_str();
      cases += kase.str();
    }
    return cases;
  }
