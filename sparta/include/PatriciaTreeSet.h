...
inline boost::intrusive_ptr<PatriciaTree<IntegerType>> insert(
  ...
  if (tree->is_leaf()) {
    ...
    return join<IntegerType>(
        key, new PatriciaTreeLeaf<IntegerType>(key), leaf->key(), leaf);
  }
  const auto& branch =
      boost::static_pointer_cast<PatriciaTreeBranch<IntegerType>>(tree);
  if (match_prefix(key, branch->prefix(), branch->branching_bit())) {
    if (is_zero_bit(key, branch->branching_bit())) {
      ...
      return new PatriciaTreeBranch<IntegerType>(branch->prefix(),
  ...
  return join<IntegerType>(
      key, new PatriciaTreeLeaf<IntegerType>(key), branch->prefix(), branch);
}
...
inline boost::intrusive_ptr<PatriciaTree<IntegerType>> merge(
  ...
  if (m == n && p == q) {
    ...
    return new PatriciaTreeBranch<IntegerType>(p, m, new_left, new_right);
  }
  if (m < n && match_prefix(q, p, m)) {
    ...
    if (is_zero_bit(q, m)) {
      ...
      return new PatriciaTreeBranch<IntegerType>(p, m, new_left, s1);
    } else {
      auto new_right = merge(s1, t);
      if (s1 == new_right) {
        return s;
      }
      return new PatriciaTreeBranch<IntegerType>(p, m, s0, new_right);
    }
  }
  if (m > n && match_prefix(p, q, n)) {
    ...
    if (is_zero_bit(p, n)) {
      ...
      return new PatriciaTreeBranch<IntegerType>(q, n, new_left, t1);
    } else {
      auto new_right = merge(s, t1);
      if (t1 == new_right) {
        return t;
      }
      return new PatriciaTreeBranch<IntegerType>(q, n, t0, new_right);
    }
  }
