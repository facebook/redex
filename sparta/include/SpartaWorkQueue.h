...
namespace spartaworkqueue_impl {
 ...
 */
template <class Input>
SpartaWorkQueue<Input> SpartaWorkQueue_foreach(
    const std::function<void(Input)>& func,
    unsigned int num_threads = sparta_parallel::default_num_threads()) {
...
void SpartaWorkQueue<Input>::run_all() {
  ...
  auto worker = [&](SpartaWorkerState<Input>* state, size_t state_idx) {
    auto attempts =
        spartaworkqueue_impl::create_permutation(m_num_threads, state_idx);
    while (true) {
