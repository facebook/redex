...
TEST_F(Dex037Test, TestDoubleCustomGreeterHasCorrectStructure) {
  ...
  EXPECT_THAT(*(double_custom_greeter_cls->get_interfaces()),
              ::testing::UnorderedElementsAre(igreeter_cls->get_type(),
                                              iformal_greeter_cls->get_type()));
...
TEST_F(Dex037Test, TestSuperCallingGreeterHasCorrectStructure) {
  ...
  auto* wrap_greet_method = find_method(super_calling_greeter_cls, "wrapGreet");
  ...
  for (auto& mie : InstructionIterable(wrap_greet_method->get_code())) {
    ...
    if (insn->has_method()) {
      ...
      if (strcmp(callee_ref->get_name()->c_str(), "greet") == 0) {
        ...
        EXPECT_EQ(resolved->get_class(), igreeter_cls->get_type());
        resolved =
            resolve_method(callee_ref, MethodSearch::Super, wrap_greet_method);
        ASSERT_NE(nullptr, resolved);
        EXPECT_EQ(resolved->get_class(), igreeter_cls->get_type());
        resolved = resolve_method(callee_ref, MethodSearch::Super);
        ASSERT_NE(nullptr, resolved);
...
TEST_F(Dex037Test, TestMidLayer2HasCorrectStructure) {
  auto* midlayer2_cls = find_class("Lcom/facebook/redextest/Dex037$MidLayer2;");
...
TEST_F(Dex037Test, TestMidLayer3HasCorrectStructure) {
  auto* midlayer3_cls = find_class("Lcom/facebook/redextest/Dex037$MidLayer3;");
...
TEST_F(Dex037Test, TestFinalLayer4HasCorrectStructure) {
  ...
  auto* ilayer_override_cls =
      find_class("Lcom/facebook/redextest/Dex037$ILayerOverride;");
  ASSERT_NE(nullptr, ilayer_override_cls);
  EXPECT_THAT(*(finallayer4_cls->get_interfaces()),
              ::testing::UnorderedElementsAre(ilayer_override_cls->get_type()));
...
TEST_F(Dex037Test, TestMidLayer4HasCorrectStructure) {
  auto* midlayer4_cls = find_class("Lcom/facebook/redextest/Dex037$MidLayer4;");
...
TEST_F(Dex037Test, TestFinalLayer4_3HasCorrectStructure) {
  ...
  auto* ilayer_conflict_cls =
      find_class("Lcom/facebook/redextest/Dex037$ILayerConflict;");
  ASSERT_NE(nullptr, ilayer_conflict_cls);
  EXPECT_THAT(*(finallayer4_3_cls->get_interfaces()),
  ...
  for (auto& mie : InstructionIterable(get_layer_method->get_code())) {
    ...
    if (insn->has_method()) {
      ...
      if (strcmp(callee_ref->get_name()->c_str(), "layer") == 0) {
        ...
        EXPECT_EQ(resolved->get_class(), ilayer_override_cls->get_type());
        resolved =
            resolve_method(callee_ref, MethodSearch::Virtual, get_layer_method);
        ASSERT_NE(nullptr, resolved);
        EXPECT_EQ(resolved->get_class(), ilayer_override_cls->get_type());
  ...
  for (auto& mie : InstructionIterable(get_layer_method->get_code())) {
    ...
    if (insn->has_method()) {
      ...
      if (strcmp(callee_ref->get_name()->c_str(), "layer") == 0) {
        EXPECT_EQ(insn->opcode(), OPCODE_INVOKE_SUPER);
        auto* resolved = resolve_invoke_method(insn);
        EXPECT_EQ(callee_ref->get_class(), ilayer_no_override_cls->get_type());
        ASSERT_NE(nullptr, resolved);
        EXPECT_EQ(resolved->get_class(), ilayer_cls->get_type());
        resolved = resolve_super(ilayer_no_override_cls, callee_ref->get_name(),
                                 callee_ref->get_proto(), nullptr);
        ASSERT_NE(nullptr, resolved);
        EXPECT_EQ(resolved->get_class(), ilayer_cls->get_type());
        resolved =
...
TEST_F(Dex037Test, TestFinalLayer10HasCorrectStructure) {
  ...
  EXPECT_THAT(
      *(finallayer10_cls->get_interfaces()),
      ::testing::UnorderedElementsAre(ilayer_override_child2_cls->get_type()));
  ...
  for (auto& mie : InstructionIterable(code)) {
    ...
    if (insn->has_method()) {
      ...
      if (strcmp(callee_ref->get_name()->c_str(), "greet") == 0) {
        ...
        ASSERT_NE(nullptr, resolved);
        EXPECT_EQ(resolved->get_class(), igreeter_cls->get_type());
      } else if (strcmp(callee_ref->get_name()->c_str(), "nestedgreet") == 0) {
        EXPECT_EQ(insn->opcode(), OPCODE_INVOKE_VIRTUAL);
        EXPECT_EQ(callee_ref->get_class(), simple_greeter_cls->get_type());
        ASSERT_NE(nullptr, resolved);
        EXPECT_EQ(resolved->get_class(), igreeter_cls->get_type());
      } else if (strcmp(callee_ref->get_name()->c_str(), "nonDefaultString") ==
                 0) {
        EXPECT_EQ(insn->opcode(), OPCODE_INVOKE_VIRTUAL);
        EXPECT_EQ(callee_ref->get_class(), simple_greeter_cls->get_type());
        ASSERT_NE(nullptr, resolved);
...
TEST_F(Dex037Test, TestResolveMethodInvokeInterfaceOnInterface) {
  ...
  auto* test_method = find_method(dex037_cls, "testInvokeInterfaceOnInterface");
  ASSERT_NE(nullptr, test_method);
...
TEST_F(Dex037Test, TestResolveMethodInvokeVirtualOnParentClass) {
  ...
  auto* test_method = find_method(dex037_cls, "testInvokeVirtualOnParentClass");
  ASSERT_NE(nullptr, test_method);
...
TEST_F(Dex037Test, TestResolveMethodInvokeVirtualWithOverride) {
  ...
  auto* test_method = find_method(dex037_cls, "testInvokeVirtualWithOverride");
  ASSERT_NE(nullptr, test_method);
  ...
  for (auto& mie : InstructionIterable(code)) {
    ...
    if (insn->has_method()) {
      ...
      if (strcmp(callee_ref->get_name()->c_str(), "greet") == 0) {
        ...
        ASSERT_NE(nullptr, resolved);
        EXPECT_EQ(resolved->get_class(), double_custom_greeter_cls->get_type());
      } else if (strcmp(callee_ref->get_name()->c_str(), "nestedgreet") == 0) {
        EXPECT_EQ(insn->opcode(), OPCODE_INVOKE_VIRTUAL);
        EXPECT_EQ(callee_ref->get_class(),
                  double_custom_greeter_cls->get_type());
  ...
  for (auto& mie : InstructionIterable(code)) {
    ...
    if (insn->has_method()) {
      ...
      if (strcmp(callee_ref->get_name()->c_str(), "greet") == 0) {
        ...
        ASSERT_NE(nullptr, resolved);
        EXPECT_EQ(resolved->get_class(), super_calling_greeter_cls->get_type());
      } else if (strcmp(callee_ref->get_name()->c_str(), "nestedgreet") == 0) {
        EXPECT_EQ(insn->opcode(), OPCODE_INVOKE_VIRTUAL);
        EXPECT_EQ(callee_ref->get_class(),
                  super_calling_greeter_cls->get_type());
...
TEST_F(Dex037Test, TestResolveMethodLayeredInvokeSuperWithInheritedDefault2_2) {
  ...
  auto* test_method =
      find_method(dex037_cls, "testLayeredInvokeSuperWithInheritedDefault2_2");
  ASSERT_NE(nullptr, test_method);
...
TEST_F(Dex037Test, TestResolveMethodLayeredInvokeSuperWithInheritedDefault2_3) {
  ...
  auto* test_method =
      find_method(dex037_cls, "testLayeredInvokeSuperWithInheritedDefault2_3");
  ASSERT_NE(nullptr, test_method);
...
TEST_F(Dex037Test, TestResolveMethodLayeredInvokeSuperWithInheritedDefault2_4) {
  ...
  auto* test_method =
      find_method(dex037_cls, "testLayeredInvokeSuperWithInheritedDefault2_4");
  ASSERT_NE(nullptr, test_method);
