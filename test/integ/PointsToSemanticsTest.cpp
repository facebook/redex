...
// Different versions of the JDK generate slightly different method semantics.
// We can't reliably tell the version of JDK in use. The test passes if the test
// result equals the result corresponding to any JDK version.
std::array<std::set<std::string>, 2u> get_expected_method_semantics() {
  // Method semantics shared by all JDK versions.
  const std::set<std::string> common_method_semantics = {
      ...
      " THIS.{D}Lcom/facebook/redextest/PointsToSemantics$Base;#<init>(0 => "
      "V1)\n"
      "}\n",
      // X#g()
      ...
      "Lcom/facebook/redextest/PointsToSemantics;#nativeMethod: ()[I = "
      "NATIVE\n",
  ...
  std::array<std::set<std::string>, 2u> method_semantics{
      common_method_semantics, common_method_semantics};

  // JDK 21+
  method_semantics[0].emplace(
      ...
      " ARRAY_ELEM(V1) = V3\n"
      " V4 = "
      "THIS.{V}Lcom/facebook/redextest/PointsToSemantics;#nativeMethod()\n"
      " V5 = GET_CLASS(V4)\n"
      " ARRAY_ELEM(V1) = V5\n"
      " V6 = NEW Ljava/util/HashSet;\n"
      " V6.{D}Ljava/util/HashSet;#<init>()\n"
      " V7 = V6.{I}Ljava/util/Set;#getClass()\n"
      " ARRAY_ELEM(V1) = V7\n"
...
const std::array<std::set<std::string>, 2u> expected_method_semantics =
    get_expected_method_semantics();
...
TEST_F(PointsToSemanticsTest, semanticActionGeneration) {
  ...
  EXPECT_THAT(expected_method_semantics, Contains(pt_output));
