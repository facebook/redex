...
class GetDeobfuscatedTest : public RedexTest {};

// DexField

TEST_F(GetDeobfuscatedTest, testField) {
...
TEST_F(GetDeobfuscatedTest, testFieldDuplicate) {
  ...
  EXPECT_EQ("foo", field->get_deobfuscated_name());
  EXPECT_EQ(field, get_result);
}

// Dex Method

TEST_F(GetDeobfuscatedTest, testMethod) {
  auto method =
