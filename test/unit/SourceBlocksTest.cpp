...
TEST_F(SourceBlocksTest, minimal_serialize) {
  ...
  auto res = insert_source_blocks(method, &cfg, false, {},
                                  /*serialize=*/true);
...
TEST_F(SourceBlocksTest, complex_deserialize) {
  ...
  auto res = insert_source_blocks(method, &cfg, false, profile,
                                  /*serialize=*/true);
...
TEST_F(SourceBlocksTest, complex_deserialize_global_default) {
  ...
  auto res = insert_source_blocks(method, &cfg, true, profile,
                                  /*serialize=*/true);
...
B4: LFoo;.bar:()V@3(x))";
  ...
  // Nothing parseable as float (and not 'x').
  {
    ...
    try {
      insert_source_blocks(method, &cfg, false, profile, /*serialize=*/true);
...
TEST_F(SourceBlocksTest, inline_normalization) {
  ...
  auto res = insert_source_blocks(foo_method, &foo_cfg, false, foo_profile,
                                  /*serialize=*/true);
  EXPECT_TRUE(res.profile_success);
  ...
  auto bar_res = insert_source_blocks(bar_method, &bar_cfg, false, bar_profile,
                                      /*serialize=*/true);
...
TEST_F(SourceBlocksTest, serialize_exc_injected) {
  ...
  auto& foo_cfg = foo_method->get_code()->cfg();
  auto res =
      insert_source_blocks(foo_method, &foo_cfg, false, {},
...
TEST_F(SourceBlocksTest, deserialize_exc_injected) {
  ...
  auto res =
      insert_source_blocks(foo_method, &foo_cfg, false, profile,
                           /*serialize=*/true, /*insert_after_excs=*/true);
  EXPECT_TRUE(res.profile_success);
  EXPECT_EQ(res.serialized, "(0(1)(2)(3) g(4) b(5 g))");
  EXPECT_EQ(
...
TEST_F(SourceBlocksTest,
  ...
  auto res = source_blocks::insert_source_blocks(
      foo_method, &foo_method->get_code()->cfg(), false, {},
      /*serialize=*/true, true);
