...
class BundleResourcesTest : public ::testing::Test {
 private:
  redex::TempDir tmp_dir;
 ...
 public:
  ...
  BundleResourcesTest()
      : tmp_dir(redex::make_tmp_dir("BundleResourcesTest%%%%%%%%")),
        extract_dir(tmp_dir.path),
        resources(prepare_resources(tmp_dir)) {}

 private:
  static BundleResources prepare_resources(const redex::TempDir& tmp_dir) {
    ...
    return BundleResources{tmp_dir.path};
  }
};

TEST_F(BundleResourcesTest, TestReadManifest) {
...
TEST_F(BundleResourcesTest, TestReadManifestClasses) {
  ...
  auto public_activity = find_component_info(manifest_info.component_tags,
                                             "Lcom/fb/bundles/PublicActivity;");
...
TEST_F(BundleResourcesTest, TestCollectRidsFromXmlAttrs) {
  auto rids = resources.get_xml_reference_attributes(
      extract_dir + "/base/manifest/AndroidManifest.xml");
  // @string/app_name, @drawable/icon and @style/ThemeA
  EXPECT_EQ(rids.size(), 3);
}

// Test collecting resource ids from xml attributes.
TEST_F(BundleResourcesTest, TestCollectResFilesByRid) {
  auto res_table = resources.load_res_table();
...
TEST_F(BundleResourcesTest, ReadLayout) {
  ...
  EXPECT_EQ(count_strings(layout_classes, "com.fb.bundles.WickedCoolButton"),
            1);
  EXPECT_EQ(count_strings(layout_classes, "com.fb.bundles.NiftyViewGroup"), 1);
...
TEST_F(BundleResourcesTest, ReadLayoutResolveRefs) {
...
TEST_F(BundleResourcesTest, RenameLayout) {
...
TEST_F(BundleResourcesTest, ReadResource) {
  auto res_table = resources.load_res_table();
  ...
  EXPECT_EQ(prickly_ids.size(), 1);
  EXPECT_EQ(
      res_table->resource_value_identical(padding_left_ids[0], prickly_ids[0]),
      false);
  ...
  EXPECT_EQ(res_table->resource_value_identical(foo_ids[0], bar_ids[0]), true);
  EXPECT_EQ(res_table->resource_value_identical(bar_ids[0], far_ids[0]), false);
  EXPECT_EQ(res_table->resource_value_identical(baz_ids[0], boo_ids[0]), false);
  ...
  EXPECT_NE(
      res_table_pb->get_hash_from_values(
          id_to_configvalue.at(padding_left_ids[0])),
      res_table_pb->get_hash_from_values(id_to_configvalue.at(prickly_ids[0])));
  EXPECT_EQ(
...
TEST_F(BundleResourcesTest, WriteResource) {
  auto res_table = resources.load_res_table();
  auto padding_left_ids = res_table->get_res_ids_by_name("padding_left");
  EXPECT_EQ(padding_left_ids.size(), 1);
...
TEST_F(BundleResourcesTest, ChangeResourceIdInLayout) {
  auto res_table = resources.load_res_table();
  auto margin_top_ids = res_table->get_res_ids_by_name("margin_top");
  EXPECT_EQ(margin_top_ids.size(), 1);
  ...
  auto changed = resources.remap_xml_reference_attributes(
      extract_dir + "/base/res/layout/activity_main.xml", kept_to_remapped_ids);
  EXPECT_EQ(changed, 4);
}

TEST_F(BundleResourcesTest, ObfuscateResourcesName) {
  auto res_table = resources.load_res_table();
  ...
  filepath_old_to_new["base/res/drawable-mdpi-v4/icon.png"] = "base/res/a.png";
...
TEST_F(BundleResourcesTest, GetConfigurations) {
  auto res_table = resources.load_res_table();
...
TEST_F(BundleResourcesTest, GetConfigsWithValue) {
  auto res_table = resources.load_res_table();
  EXPECT_EQ(res_table->package_count(), 1);
...
TEST_F(BundleResourcesTest, GetOverlayableRootIds) {
  auto res_table = resources.load_res_table();
...
TEST_F(BundleResourcesTest, TestRemoveStyleAttribute) {
  ...
  for (const auto& mod : modifications) {
    ...
    EXPECT_THAT(new_style_resources[0].attributes, Not(Contains(Key(attr_id))))
...
TEST_F(BundleResourcesTest, TestAddStyleAttribute) {
  ...
  for (const auto& mod : modifications) {
    ...
    if (attr_id == kTextStyleAttrId) {
      ...
      EXPECT_EQ(added_attr.get_data_type(), android::Res_value::TYPE_REFERENCE);
      ...
      EXPECT_EQ(added_attr.get_data_type(), android::Res_value::TYPE_DIMENSION);
      EXPECT_EQ(added_attr.get_value_bytes(), 0x00000064);
    } else if (attr_id == kFractionAttrId) {
      EXPECT_EQ(added_attr.get_data_type(), android::Res_value::TYPE_FRACTION);
    ...
    for (uint32_t original_attr_id :
      ...
      EXPECT_THAT(new_style_resources[0].attributes,
          ...
          << "Original attribute 0x" << std::hex << original_attr_id << std::dec
          << " is missing from style 0x" << std::hex << resource_id;
    }
  }
}

TEST_F(BundleResourcesTest, TestRemoveAndAddStyleAttributes) {
  using Value = resources::StyleResource::Value;
  auto res_table = resources.load_res_table();
  const auto& paths = resources.find_resources_files();
  auto style_map = res_table->get_style_map();
...
TEST_F(BundleResourcesTest, TestResourceExists) {
...
TEST_F(BundleResourcesTest, TestApplyStyleMerges) {
  ...
  auto modifications = pass.get_parent_and_attribute_modifications_for_merging(
...
TEST_F(BundleResourcesTest, TestApplyStyleChained) {
  ResourceValueMergingPass pass;
...
TEST_F(BundleResourcesTest, TestAddStyles) {
  auto res_table = resources.load_res_table();
  const auto& paths = resources.find_resources_files();
