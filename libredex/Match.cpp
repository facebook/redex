...
bool default_constructor(const DexMethod* meth) {
  if (!is_static(meth) && is_constructor(meth) && has_no_args(meth) &&
      has_code(meth)) {
...
match_t<IRInstruction, std::tuple<int>> has_n_args(int n) {
  return {// N.B. "int n" must be const ref in order to appease N-ary matcher
          // template
...
match_t<DexClass, std::tuple<>> is_interface() {
  return {[](const DexClass* cls) {
    return (bool)(cls->get_access() & ACC_INTERFACE);
  }};
}

match_t<IRInstruction> has_type() {
  return {[](const IRInstruction* insn) { return insn->has_type(); }};
}
...
match_t<DexMethod, std::tuple<>> is_default_constructor() {
  return {[](const DexMethod* meth) { return default_constructor(meth); }};
}

match_t<DexMethodRef, std::tuple<>> can_be_default_constructor() {
  return {[](const DexMethodRef* meth) {
    return meth->is_def() &&
...
match_t<DexMethod, std::tuple<>> is_constructor() {
  return {[](const DexMethod* meth) { return is_constructor(meth); }};
}

match_t<DexMethodRef, std::tuple<>> can_be_constructor() {
  return {[](const DexMethodRef* meth) { return is_constructor(meth); }};
}
...
match_t<DexClass, std::tuple<>> has_class_data() {
  return {[](const DexClass* cls) { return cls->has_class_data(); }};
}
...
match_t<DexType, std::tuple<const DexType*>> is_assignable_to(
  ...
  return {[](const DexType* child, const DexType* const& parent) {
