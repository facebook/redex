...
void MultiMethodInliner::inline_callees(
  ...
  ir_list::InstructionIterable ii(caller->get_code());
  auto end = ii.end();
...
bool MultiMethodInliner::cannot_inline_opcodes(const DexMethod* caller,
                                               const DexMethod* callee) {
  int ret_count = 0;
  for (auto& mie : ir_list::InstructionIterable(callee->get_code())) {
    auto insn = mie.insn;
...
std::unique_ptr<RegMap> gen_callee_reg_map(
  ...
  auto param_insns = ir_list::InstructionIterable(callee_code->get_param_instructions());
