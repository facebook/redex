...
namespace redex {
  namespace proguard_parser {
    ...
    bool is_identifier_character(char ch) {
      return isalnum(ch) || ch == '_' || ch == '$' || ch == '*' || ch == '.' || ch == '\'' || ch == '[' || ch == ']' || ch == '<' || ch == '>' || ch == '!' || ch == '?' || ch == '%';
    }
    ...
    vector<string> read_paths(istream& config, unsigned int* line) {
      vector<string> paths;
      ...
      while (config >> noskipws >> ch) {
        ...
        if (ch == '-') {
          ...
          if (command == "keepclasseswithmembernames") {
            tokens.push_back(unique_ptr<Token>(new KeepClassesWithMemberNames(line)));
            continue;
          }
          ...
          if (command == "mergeinterfacesaggressively") {
            tokens.push_back(unique_ptr<Token>(new MergeInterfacesAggressively(line)));
            continue;
          }
          ...
          if (command == "repackageclasses") {
            ...
            if (package_name != "") {
              tokens.push_back(unique_ptr<Token>(new Identifier(line, package_name)));
            }
            continue;
          }
          ...
          if (command == "dontusemixedcaseclassnames") {
            tokens.push_back(unique_ptr<Token>(new DontUseMixedcaseClassNames(line)));
            continue;
          }
        ...
        if (word == "interface") {
          ...
          if (!tokens.empty() && tokens.back()->type == token::annotation_application) {
