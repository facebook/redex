...
void Iterator::adjust_block() {
  while ((m_block != nullptr) && m_it == m_block->end()) {
    m_block = m_block->goes_to();
    if ((m_block != nullptr) &&
        is_big_block_successor(m_block, m_ignore_throws)) {
      m_it = m_block->begin();
    } else {
      m_block = nullptr;
    }
...
InstructionIterator& InstructionIterator::operator++() {
  ...
  while (true) {
    ...
    if ((next_block == nullptr) ||
        !is_big_block_successor(next_block, m_ignore_throws)) {
...
InstructionIterator InstructionIterable::end() const {
  const auto& last_block = m_big_block.get_blocks().back();
...
boost::optional<BigBlock> get_big_block(cfg::Block* block) {
  ...
  do {
    ...
    block = block->goes_to();
  } while ((block != nullptr) && is_big_block_successor(block));
